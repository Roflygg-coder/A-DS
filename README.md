...
Task 5:
    *The longest common subsequence with recovery*
...
Task 10:
    *A backpack with the recovery of the answer (C)*
...
Task 11:
    *Pizza*
    ...
    Input data
    The numbers A,B,C, N are entered first,
    and then N
    numbers d1,...,dN
    . All numbers are integers, 1≤A≤1000.1≤B≤1000.1≤C≤1000.0≤N≤1000.1≤di≤1000000
    .
    ...
    Note
    In the first example, it is most economical to purchase 1, 2 and 5 products. In the second case, you do not need to buy anything, because the delivery has already become free. In the third case, it's cheaper to pay for the delivery yourself.
...
Task 15:
    *Painting the fence*
    Vasya's grandfather built a fence at the cottage from whatever came to hand. The fence consists of a row of N
    boards 10 cm wide, but possibly of different heights.
...
Task 16:
    *Sequences of 0 and 1 without k identical characters in a row*
...
Task 23:
    *Weight: stacks of the same size*
    ...
    should be output in the first line of the kettlebell numbers (numbers from 1 to N) included in the first pile,
    in the second line — the kettlebell numbers in the second pile.
...
Task 1:
    *All strings of length n of k different characters*
...
Task 2:
    *All binary strings of length n containing exactly k units*
...
Task 3:
    *All decreasing sequences of length k from the numbers 1..n*
...
Task 8:
    ...
    Note
    Correct solutions for tests in which n<1000
    and k=2
    are evaluated out of 30 points.

    Correct solutions for tests in which k=2
    are estimated from 60 points (these scores also include 30 points for the case n≤1000, k=2).
...
Task 6:
    *Elevator in the business center*
...
Task 13:
    *A lonely number*
    ...
    requests. In order to make a request, your solution should output it in the following format: "? i" (without quotes,1≤ i ≤ N ). The answer to the query is the number
    a(i).
